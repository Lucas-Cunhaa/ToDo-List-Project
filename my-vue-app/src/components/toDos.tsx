...
const TaskGroup = ({ title, tasks, handleChange }: { title: string, tasks: Task[], handleChange: (id: number, newState: string) => void }) => (
      ...
      <div key={id} className={`task-group ${state.toLowerCase()}`}>
        <div className={`task-left ${state.toLowerCase()}-left`}>
          <div className={`task-title ${state.toLowerCase()}-title`}>
            ...
            <h2 className={`task-title-h2 ${state.toLowerCase()}-title-h2`}>{title}</h2>
          </div>
          <div className={`task-description ${state.toLowerCase()}-description`}>
            <h3 className={`task-description-h3 ${state.toLowerCase()}-description-h3`}>{description}</h3>
          </div>
        </div>
        <div className={`task-right ${state.toLowerCase()}-right`}>
          <div className="select-group">
            <select id="select-state" className="to-do-select-state" value={state} onChange={(e) => handleChange(id, e.target.value)}>
            ...
            <svg className="delete-small-svg" width="12" height="12" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M7.40725 0.403931L0.643799 7.16738M0.643799 0.403931L7.40725 7.16738" stroke="#101112" strokeWidth="0.737831"/>
            </svg>
          </div>
          <div className={`task-member ${state.toLowerCase()}-member`}>
            <svg className="member-svg" width="24" height="24" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M1.09766 13.3878C1.09766 11.4664 3.65957 11.4664 4.94053 10.1854C5.581 9.54492 3.65957 9.54492 3.65957 6.34253C3.65957 4.20781 4.51333 3.14014 6.22148 3.14014C7.92964 3.14014 8.78339 4.20781 8.78339 6.34253C8.78339 9.54492 6.86196 9.54492 7.50244 10.1854C8.78339 11.4664 11.3453 11.4664 11.3453 13.3878" stroke="black" strokeWidth="0.614859" strokeLinecap="round"/>
            </svg>
            <h2 className={`task-member-h2 ${state.toLowerCase()}-member-h2`}>{member}</h2>
          </div>
        </div>
      </div>
    ))}
